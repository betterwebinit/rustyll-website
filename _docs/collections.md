...
talks at a conference. Rustyll offers enhanced performance for collections through
parallel processing.
...
<div class="note">
  ...
  <p>Then Rustyll will look in <code>my_collections/_books</code> for the <code>books</code> collection, and
...
Rustyll can process collections in parallel for significantly improved build times. To enable parallel processing for a collection:

```yaml
collections:
  staff_members:
    parallel: true
    threads: 4  # Optional: specify number of threads (defaults to auto)
    cache: aggressive  # Optional: specify caching strategy
```

This is particularly useful for large collections with many documents.
...
matter is provided, Rustyll will consider it to be a [static file]({{ '/docs/static-files/' | relative_url }})
and the contents will not undergo further processing. If front matter is provided,
Rustyll will process the file contents into the expected output.

Regardless of whether front matter exists or not, Rustyll will write to the destination
...
name: Jane Doe
position: Developer
parallel: true  # Process this document in parallel
cache: always   # Always use cached version when available
---
Jane has worked on Rustyll for the past *five years*.
```
...
If you'd like Rustyll to create a rendered page for each document in your
...
<div class="note info">
  ...
  <code>posts</code> collection is hard-coded into Rustyll. It exists whether
    ...
    <tr>
      ...
      <td>
        <p>
          ...
          provided, Rustyll will not generate the file in your collection. If
