...
If your Rustyll site has a lot of pages, you might want to create navigation for the pages. Instead of hard-coding navigation links, you can programmatically retrieve a list of pages to build the navigation for your site.

Although there's already information about [interacting with data files]({% link _docs/datafiles.md %}) in other Rustyll docs, this tutorial dives into building more robust navigation for your site.

There are two primary ways of retrieving pages on a Rustyll site:
...
When building navigation for large sites with many pages, Rustyll's parallel processing capabilities can significantly improve build times. To take advantage of this:

1. Enable parallel processing in your `_config.yml`:
...
2. For complex navigation structures, consider using data files (as shown in this tutorial) rather than querying all site pages directly. This approach is more cache-friendly and can be processed more efficiently in parallel.

3. If your navigation includes many elements or nested levels, Rustyll's optimized Liquid rendering will process them faster than traditional Jekyll, especially when parallel processing is enabled.
  ...
  - title: Deployment
    ...
    subnav:
      - title: Heroku
        url: heroku.html
        subnav:
          - title: Rustyll on Heroku
            url: rustyll-on-heroku.html
  - title: Help
    url: help.html
```
...
<div class="highlight result" data-proofer-ignore>
   <ul>
      ...
      <ul>
        ...
        <ul>
          <li><a href="#">Rustyll on Heroku</a></li>
        </ul>
      </ul>
      <li><a href="#">Help</a></li>
   </ul>
